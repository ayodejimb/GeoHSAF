...
splits_root = r"C:\.....\ADNI\After_matching\Splitted_triple"
folds = sorted([f for f in os.listdir(splits_root) if f.startswith("fold")])
for fold in folds:

    print(f"------ NOW RUNNING FOR ------{fold}")

    condis_train = ['CN_train', 'AD_train', 'MCI_train']
    condis_test = ['CN_test', 'AD_test', 'MCI_test']

    data_folder_CN = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_PCA_tangent_data'.format(fold, condis_train[0]) 
    data_folder_AD = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_PCA_tangent_data'.format(fold, condis_train[1])
    data_folder_MCI = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_PCA_tangent_data'.format(fold, condis_train[2])  

    data_folder_interp_CN_train = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_train[0]) 
    data_folder_interp_AD_train = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_train[1])
    data_folder_interp_MCI_train = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_train[2])  
    data_folder_interp_CN_test = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_test[0]) 
    data_folder_interp_AD_test = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_test[1])
    data_folder_interp_MCI_test = r'C:\.....\ADNI\After_matching\Splitted_triple\{}\{}_interpolated_PCA_tangent_data'.format(fold, condis_test[2])
    ...
    subject_CN_dict_train = load_interpolated_subject_time_series(data_folder_interp_CN_train, time_map_CN)
    subject_AD_dict_train = load_interpolated_subject_time_series(data_folder_interp_AD_train, time_map_AD)
    subject_CN_dict_test = load_interpolated_subject_time_series(data_folder_interp_CN_test, time_map_CN)
    subject_AD_dict_test = load_interpolated_subject_time_series(data_folder_interp_AD_test, time_map_AD)
    subject_MCI_dict_train = load_interpolated_subject_time_series(data_folder_interp_MCI_train, time_map_MCI)
    subject_MCI_dict_test = load_interpolated_subject_time_series(data_folder_interp_MCI_test, time_map_MCI)
    ...
    dim5 = get_first_array_dimension(subject_MCI_dict_test)
    dim6 = get_first_array_dimension(subject_MCI_dict_train)

    print(dim1, dim2, dim3, dim4, dim5, dim6)

    max_dim = max(dim1, dim2, dim3, dim4, dim5, dim6)
    ...
    subject_MCI_dict_test = pad_dict_arrays(subject_MCI_dict_test, max_dim)
    subject_MCI_dict_train = pad_dict_arrays(subject_MCI_dict_train, max_dim)
    ...
    def prepare_subject_data(group_dict, label):
        ...
        for subject_id, time_dict in group_dict.items():
            ...
            for t in times:
                ...
                vec = vec.squeeze()  # remove any singleton dimension
                if vec.ndim != 1:
    ...
    AD_data_train = prepare_subject_data(subject_AD_dict_train, label=2)
    MCI_data_train = prepare_subject_data(subject_MCI_dict_train, label=0)
    all_data_train = CN_data_train + AD_data_train + MCI_data_train 

    CN_data_test = prepare_subject_data(subject_CN_dict_test, label=1)
    AD_data_test = prepare_subject_data(subject_AD_dict_test, label=2)
    MCI_data_test = prepare_subject_data(subject_MCI_dict_test, label=0)
    all_data_test = CN_data_test + AD_data_test + MCI_data_test

    def extrac_tensors(data, components=5):
        trimmed_data = []
        for sequence, label in data:
            trimmed_sequence = [t[:components] for t in sequence]
            trimmed_data.append([trimmed_sequence, label])
        return trimmed_data

    all_data_train= extrac_tensors(all_data_train, components=250)
    all_data_test= extrac_tensors(all_data_test, components=250)
    ...
    def compute_pca_scale(train_loader, num_batches=16):
        vals = []
        for i, batch in enumerate(train_loader):
            if isinstance(batch, (list, tuple)):
                ...
                x = batch   
            vals.append(x.abs().mean().item())
            if i >= num_batches:
                break
        return sum(vals) / len(vals)

    def get_adaptive_scheduler(optimizer, warmup_steps, total_steps, pca_scale, base_lr=1e-4):
        scale_factor = max(pca_scale / 1e-3, 1e-4)  # keeps LR in safe range
        ...
        for g in optimizer.param_groups:
            g["lr"] = scaled_lr  # set initial LR
        
        return LambdaLR(optimizer, lr_lambda)
        
    pca_scale = compute_pca_scale(train_loader)
    print(f"Estimated PCA scale: {pca_scale:.2e}")


    input_dim = all_data_train[0][0][0].shape[0]  # get the size of the PCA vectors
    model = TransformerClassifier(input_dim=input_dim, model_dim=128, num_classes=3).to(device)
    ...
    def train_one_epoch(model, dataloader, optimizer, criterion, scheduler, num_classes=3):
        model.train()
    ...
    for epoch in range(epochs):
        train_loss, train_acc, train_auc = train_one_epoch(model, train_loader, optimizer, criterion, scheduler)
